...
public class AVLTreeNode<T extends Comparable<T>> {
	...
	int balance = nodeHeight(left) - nodeHeight(right); //negative --> right heavy ...
	if (balance < -1) {
		//right heavy --> gör något (left, right-left adjust)
		if (right.element.compareTo(element) < 0) {
			//right-left-case --> right-left rotate
			return doubleRotationWithRightChild(this);
		} else if (element.compareTo(right.element) < 0) {
			//right-right-case --> left rotate
			return singleRotationWithRightChild(this);
		}	
		
	} else if (balance > 1) {
		//left heavy --> (right eller left-right)
		if (element.compareTo(left.element) < 0) {
			//left-right-case --> left-right rotate
			return doubleRotationWithLeftChild(this);
		} else if (left.element.compareTo(element) < 0) {
